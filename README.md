...
COMFY_VIDEO_CLIP_NAME=umt5_xxl_fp8_e4m3fn_scaled.safetensors
...
- if error shows `unet_name ... not in []`, Comfy has no video UNETs discovered; add files under `ComfyUI/models/diffusion_models`
- if error shows `clip_name ... not in []`, Comfy has no text encoders discovered; add files under `ComfyUI/models/text_encoders`
